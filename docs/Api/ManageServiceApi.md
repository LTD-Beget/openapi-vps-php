...
```php
manageServiceGetSoftwareList(): \OpenAPI\Client\Model\ManageGetSoftwareListResponse
```
...
try {
    $result = $apiInstance->manageServiceGetSoftwareList();
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling ManageServiceApi->manageServiceGetSoftwareList: ', $e->getMessage(), PHP_EOL;
}
```
...
[**\OpenAPI\Client\Model\ManageGetSoftwareListResponse**](../Model/ManageGetSoftwareListResponse.md)
